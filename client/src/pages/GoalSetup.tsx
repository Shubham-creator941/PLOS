    ...
    <div className="mx-auto max-w-3xl py-8">
      ...
      <div className="mb-12">
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800">
          <div
            className="h-full bg-primary-600 transition-all duration-500 ease-in-out dark:bg-primary-500"
        ...
        return (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {styles.map((style) => (
              <label
                key={style}
                className={`flex cursor-pointer items-center justify-between rounded-xl border p-4 transition-all hover:bg-neutral-50 dark:hover:bg-neutral-800 ${
                  field.value.includes(style)
                    ? 'border-primary-500 bg-primary-50/50 dark:border-primary-500 dark:bg-primary-900/20'
                    : 'border-neutral-200 dark:border-neutral-800'
                }`}
              >
                <span className="font-medium text-neutral-900 dark:text-neutral-50">{style}</span>
                <input
                  type="checkbox"
                  className="hidden"
                ...
                <div className={`flex h-5 w-5 items-center justify-center rounded-full border ${
                  field.value.includes(style) 
                   ...
                   {field.value.includes(style) && <CheckCircle2 className="h-3 w-3 text-white" strokeWidth={3} />}
                </div>
              </label>
            ))}
...
const FinalReview = ({ formData }: { formData: GoalSetupFormValues }) => (
  <div className="space-y-8">
    <div className="text-center">
      <h2 className="mb-2 text-3xl font-bold tracking-tight text-neutral-900 dark:text-neutral-50">Review Your Journey</h2>
      <p className="text-neutral-500 dark:text-neutral-400">Everything looks great. Ready to start?</p>
    </div>

    <Card className="overflow-hidden border-2 border-primary-100 dark:border-primary-900/30">
      <div className="bg-primary-50 p-6 dark:bg-primary-900/20">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-primary-600 dark:text-primary-400">Identity</h3>
        <p className="mt-2 text-xl font-medium italic text-neutral-900 dark:text-neutral-50">"{formData.identity}"</p>
      </div>
      
      <CardContent className="divide-y divide-neutral-100 p-0 dark:divide-neutral-800">
        <div className="p-6">
          <h4 className="text-xs font-semibold uppercase text-neutral-500">The Goal</h4>
          <p className="mt-1 font-medium text-neutral-900 dark:text-neutral-50">{formData.goal}</p>
        </div>
        
        <div className="p-6">
          <h4 className="text-xs font-semibold uppercase text-neutral-500">The Why</h4>
          <p className="mt-1 text-neutral-700 dark:text-neutral-300">{formData.why}</p>
        </div>

        <div className="grid grid-cols-2 divide-x divide-neutral-100 dark:divide-neutral-800">
          <div className="p-6">
            <h4 className="text-xs font-semibold uppercase text-neutral-500">Commitment</h4>
            <p className="mt-1 font-medium text-neutral-900 dark:text-neutral-50">{formData.hoursPerWeek}h / week ({formData.preferredTime})</p>
            <p className="mt-1 text-sm text-neutral-500">{formData.sessionDuration} sessions</p>
          </div>
          <div className="p-6">
            <h4 className="text-xs font-semibold uppercase text-neutral-500">Target Date</h4>
            <p className="mt-1 font-medium text-neutral-900 dark:text-neutral-50">{new Date(formData.targetDate).toLocaleDateString()}</p>
          </div>
        </div>

        <div className="p-6">
          <h4 className="text-xs font-semibold uppercase text-neutral-500">Learning Styles</h4>
          <div className="mt-2 flex flex-wrap gap-2">
            {formData.learningStyles.map(style => (
              <span key={style} className="inline-flex items-center rounded-full bg-neutral-100 px-2.5 py-0.5 text-xs font-semibold text-neutral-800 dark:bg-neutral-800 dark:text-neutral-300">
                {style}
              </span>
            ))}
          </div>
