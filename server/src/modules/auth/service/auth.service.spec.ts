...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('should create a new learner if email does not exist', async () => {
      repoMock.emailExists.mockResolvedValue(false);
      repoMock.findById.mockResolvedValue({ learner_id: 'mock_uuid', email: 'test@example.com' } as any);
      ...
      expect(repoMock.emailExists).toHaveBeenCalledWith(dto.email);
      ...
      expect(result.learner).toHaveProperty('learner_id', 'mock_uuid');
    });

    it('should throw an error if email exists', async () => {
      repoMock.emailExists.mockResolvedValue(true);
